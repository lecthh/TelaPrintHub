...
<div class="flex flex-col gap-y-3 p-3 bg-kWhite border border-kBlack lowercase">
    <div class="flex gap-x-3">
        ...
        <h1 class="lowercase font-bold text-base text-[#5C5959]">active orders</h1>
    </div>
    <table class="border border-kBlack">
        ...
        <tbody>
            ...
            <tr>
                <td colspan="6" class="text-center p-2 text-red-500">No Active Orders</td>
            </tr>
            @else
            @foreach($ActiveOrders as $orderPlacement)
            <tr class="hover:bg-gray-100 cursor-pointer" data-url="{{ route('order-active-details', ['order_placement_ID' => $orderPlacement->order_placement_ID]) }}">
